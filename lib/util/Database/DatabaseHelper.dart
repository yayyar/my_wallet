...
class DatabaseHelper {
  ...
  Future<List> getActualCost(
      {required int startDate, required int endDate}) async {
    var dbClient = await database;
    String sql;
    if (startDate == endDate) {
      sql =
          "SELECT SUM($_actualCost) as actualCost FROM $_itemTable itb inner join $_budgetCategoryTable bct on bct.$_categoryId = itb.$_categoryId inner join $_budgetTable bt on bt.$_budgetId = bct.$_budgetId WHERE bt.$_budgetStatus = 1 AND bt.$_monthYear = itb.$_monthYear AND itb.$_itemDate = $startDate";
    } else {
      sql =
          "SELECT SUM($_actualCost) as actualCost FROM $_itemTable itb inner join $_budgetCategoryTable bct on bct.$_categoryId = itb.$_categoryId inner join $_budgetTable bt on bt.$_budgetId = bct.$_budgetId WHERE bt.$_budgetStatus = 1 AND bt.$_monthYear = itb.$_monthYear AND itb.$_itemDate BETWEEN $startDate AND $endDate";
    }
    //print(sql);
    var res = await dbClient.rawQuery(sql);
  ...
  Future<List> getAllExpenseToDownload(
      {required int startDate, required int endDate}) async {
    var dbClient = await database;
    String sql;
    if (startDate == endDate) {
      sql =
          "SELECT itb.$_itemNameDescription, ct.$_categoryName, itb.$_itemDate, itb.$_actualCost from $_itemTable itb inner join $_categoryTable ct on itb.$_categoryId = ct.$_categoryId AND itb.$_itemDate = $startDate ORDER BY itb.$_itemId ASC";
    } else {
      sql =
          "SELECT itb.$_itemNameDescription, ct.$_categoryName, itb.$_itemDate, itb.$_actualCost from $_itemTable itb inner join $_categoryTable ct on itb.$_categoryId = ct.$_categoryId AND itb.$_itemDate BETWEEN $startDate AND $endDate ORDER BY itb.$_itemId ASC";
    }
    //print(sql);
    var res = await dbClient.rawQuery(sql);
  ...
  Future<List> getExpenseItems(
    ...
    if (startDate == endDate) {
      sql =
          "SELECT itb.$_itemId,bct.$_estimateCost,itb.$_itemNameDescription, itb.$_itemDate, itb.$_actualCost from $_itemTable itb inner join $_budgetCategoryTable bct on itb.$_categoryId = bct.$_categoryId inner join $_budgetTable bt on bct.$_budgetId = bt.$_budgetId WHERE bct.$_categoryId = $categoryId AND bt.$_budgetStatus = 1 AND bt.$_monthYear = itb.$_monthYear AND itb.$_itemDate = $startDate ORDER BY itb.$_itemId ASC";
    } else {
      sql =
  ...
  Future<int> deleteExpenseItem(int id) async {
    var dbClient = await database;
    var result =
        dbClient.delete(_itemTable, where: "$_itemId = ? ", whereArgs: [id]);
    return result;
  }
  ...
  Future<int> deleteCategoryItem(int id) async {
    var dbClient = await database;
    var result = dbClient.delete(_budgetCategoryTable,
        where: "$_budgetCategoryId = ? ", whereArgs: [id]);
    return result;
  }
  ...
  Future<int> findActiveBudget() async {
    var dbClient = await database;
    String sql = "SELECT COUNT(*) FROM $_budgetTable WHERE $_budgetStatus = 1";
    var result = await dbClient.rawQuery(sql);
    int count = Sqflite.firstIntValue(result) ?? 0;
  ...
  Future<List> currentBudgetDate() async {
    var dbClient = await database;
    String sql =
        "SELECT $_budgetDate,$_lastUpdatedDate from $_budgetTable WHERE $_budgetStatus = 1 ";
    var result = await dbClient.rawQuery(sql);
    return result.toList();
  }
